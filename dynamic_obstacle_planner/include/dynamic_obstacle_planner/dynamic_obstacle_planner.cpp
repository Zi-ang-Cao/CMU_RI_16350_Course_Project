...
namespace dynamic_obstacle_planner{
    ...
    void dyn_obs_planner::plannerCB(const ros::TimerEvent&){
        ...
        if (this->has_plan_to_execute) {
            if (not this->do_rewiring) {
                printf("Either Following previous rewiring path, or being disable from rewire current plan\n");
            }
            if (achieve_temp_goal()) {
                this->do_rewiring = true;
                cout<< "achieve_temp_goal" <<endl;
            }
            else {
                // Must have "do_rewiring=True"
                if (this->near_dyn_obs) replan_for_dynamicObs=true;
        ...
        if ((not replan_for_newGoal) && (not replan_for_dynamicObs)) return;
        else update_new_plan = true;
        ...
        if (replan_for_newGoal) {
            refresh_NodeTable();
            publishPlan_pair_vec.clear();
            cout<<"Received a new goal AND generating path via A*"<<endl;            
            AStarBaseline(&publishPlan_pair_vec, curPosi, goalPosi);
            cout << "Published Path via AStarBaseline" << endl;
            // Update flags
            replan_for_newGoal = false;
            do_rewiring = true;
        }
        else if (replan_for_dynamicObs) {
            // Split origin plan into two part!
            ...
            switch (REPLAN_STRATEG_Indicator)
            {
                case do_ASTAR_Total_Replan: {
                    ...
                    Lable_avoidZone();
                    AStarBaseline(&baselinePlan_pair_vec, curPosi, goalPosi);
                ...
                case do_ASTAR_Partial_Replan: {
                    ...
                    // Create the baseline A* Path for the agent
                    while (this->curPosi != this->goalPosi)
                    {
                        AStarBaseline(&baselinePlan_pair_vec, this->curPosi, this->goalPosi);
                        if (this->near_dyn_obs)
                        {
                            // Set the is_tempGoalAchieved to false until it does achieve the temp goal
                            this->is_tempGoalAchieved == false;
                            // Find the temporary goal that is closest to the barrier and closest to the goal from the baselinePlan_pair_vec
                            tempGoalPosi = findTempPosi(baselinePlan_pair_vec, this->obs_pos_x, this->obs_pos_y, this->replan_range, goalPosi, tempGoalPosi);
                            int tempGoalIndex = find(baselinePlan_pair_vec.begin(), baselinePlan_pair_vec.end(),tempGoalPosi);
                            auto startIter = next(baselinePlan_pair_vec.begin(), tempGoalIndex);
                            auto endIter = baselinePlan_pair_vec.end();
                            publishPlan_pair_vec(startIter, endIter);
                            // Generate Untraversable Zone
                            Lable_avoidZone();
                            // Generate Temporary Path for barrier
                            AStarBaseline(&tempPlan_pair_vec, this->curPosi, this->goalPosi);
                            if (curPosi == tempGoalPosi){
                                this->is_tempGoalAchieved == true;
                                // Follow the remainder of baselinePlan_pair_vec
                            }
                        }
                    }

                    tempGoalPosi = goalPosi;
                    cout << "Need a smarter way to find TempPosi" << endl;
                    
                    AStarBaseline(&publishPlan_pair_vec, curPosi, tempGoalPosi);
        ...
        if (update_new_plan) {
            ...
            // Essential Reset!!!!
            this->PID_path_ref_index = 1;   // Essential reset!!!!
            this->pseudo_loop = 0;   // Essential reset!!!!

            // Publish to ROS MESSAGE CHANNEL
            updatePathVisVec(this->publishPlan_pair_vec);
            this->pathMsgConverter(this->publishPlan_pair_vec, this->path_msg_);
            this->pathVisMsg_.markers = this->pathVisVec_;
            this->visPathPub_.publish(this->pathVisMsg_);

            update_new_plan = false;    // Reset Flag
        }
    ...
    void dyn_obs_planner::Lable_avoidZone() {
        ...
        for (int i = 0; i<=Barrier_Resolution; i++) {
            ...
            temp_x = obs_pos_x + Barrier_Radius * cos(Barrier_Angle + Barrier_Orientation);
            temp_y = obs_pos_y + Barrier_Radius * sin(Barrier_Angle + Barrier_Orientation);


            Barrier_pair_vec.push_back(make_pair(temp_x, temp_y));
            Barrier_pair_vec.push_back(make_pair(temp_x+STEP_SIZE, temp_y+STEP_SIZE));
            Barrier_pair_vec.push_back(make_pair(temp_x-STEP_SIZE, temp_y-STEP_SIZE));
            Barrier_pair_vec.push_back(make_pair(temp_x-STEP_SIZE, temp_y+STEP_SIZE));
            Barrier_pair_vec.push_back(make_pair(temp_x+STEP_SIZE, temp_y-STEP_SIZE));
        }
    ...
    void dyn_obs_planner::UN_Lable_avoidZone() {
        ...
        Barrier_pair_vec.clear();
    }



    void dyn_obs_planner::AStarBaseline(vector<pair<double,double>>* curPlan_pair_vec,
        ...
        curPlan_pair_vec->clear();

        for (auto pair: plan) {
            curPlan_pair_vec->push_back(pair);
            // Print out the plan
    ...
    void dyn_obs_planner::pseudoRobCB(const ros::TimerEvent&){
        ...
        if (dyn_obs_isOn) {
            ...
            if (obs_loop!=0) this->obs_distance_to_Rob_buffer = this->obs_distance_to_Rob;
            else this->obs_distance_to_Rob_buffer = INF;
            ...
            if (this->obs_distance_to_Rob <= this->replan_range) {
                // Only set this->near_dyn_obs = true once --- at the time it enters the circle
                // if (this->obs_distance_to_Rob_buffer > this->replan_range) {
                //     printf("==== near_dyn_obs is on!! ====\n");
    ...
    void dyn_obs_planner::PID(const ros::TimerEvent&){
		...
		// if (endIdx != 0) {
			...
			// noice <=0.1
			if (noice2 <=0.25 && noice1 <=0.25 && PID_path_ref_index == endIdx-1 )
			{
    ...
    //     distance = pushback(sqrt(dx*dx + dy*dy));
    // }
    ...
    //         double distToObstacle = this->distance(point, make_pair(obs_pos_x, obs_pos_y));
    ...
    //         double distToGoal = this->distance(point, goalPosi);
